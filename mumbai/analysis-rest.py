...
class AnalyzeRestaurantItem(object):
	...
	db_name = 'mumbairestaurants1'
	info_fields = ['restaurant name', 'latitude', 'longitude', 'street', 'locality', 'city', \
				'state', 'country', 'rating', 'price', 'review excellent count', \
	...
	def __init__(self):
		...
		self.client = MongoClient('mongodb://localhost:27017/mumbairestaurants1')
	...
	def convert_addr_to_coord(self, addr):
		...
		location = geolocator.geocode(addr, timeout=10000)
		if location:
	...
	def load_mongodb_to_pandas(self):
		...
		rest_features = []
		rest_meals = []
		for doc in self.db.mumbairestaurants1.find():
			
			street = doc['rest_street']
			city = doc['rest_city']
			country = doc['rest_country']
			locality = doc['rest_locality']
			lat, lon = self.convert_addr_to_coord(street + ", " + city + ", " + country)
			if not lat:
				lat, lon = self.convert_addr_to_coord(locality + ", " + city + ", " + country)
			...
			if lat != 0 and lon != 0:
				...
				positive_review_count = int(doc['review_excellent_count'].replace(',','')) + \
				...
				rest_info.append([doc['rest_name'], float(lat), float(lon), doc['rest_street'],doc['rest_locality'], \
								doc['rest_city'],doc['rest_state'], doc['rest_country'], doc['rest_rating'], rest_price, \
		...
		self.info_df.to_csv('restaurant_info.csv', index=False)
