...
class ProcessRestaurantItem(object):
	...
	fields = ['rest_name', 'rest_locality', 'rest_reviews']
	df = None	
	...
	def load_mongodb_to_pandas(self):	
		...
		for doc in self.db.mumbairestaurants1.find():
			restaurants.append([doc['rest_name'], doc['rest_locality'], doc['rest_reviews']])
	...
	def generate_summary(self, rest_name, locality, freq_features, processed_reviews_df):
		feature_summary_reviews = []
		for feature in freq_features:
			for i, review in enumerate(processed_reviews_df['reviews']):
				if feature in review:
					feature_summary_reviews.append([rest_name.lower(), locality, feature, review,
								processed_reviews_df['sentiment'][i]])
		feature_summary_df = pd.DataFrame(feature_summary_reviews, 
							columns=['restaurant name', 'locality', 'feature', 'review', 'sentiment'])
	...
	def process_reviews(self):
		...
		for i, review_collection in enumerate(self.df['rest_reviews']):
			...
			locality = self.df['rest_locality'][i]
			...
			feature_summary_df = self.generate_summary(rest_name, locality, freq_features, processed_reviews_df)
